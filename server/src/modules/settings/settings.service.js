  ...
  async updateSystemSettings(data) {
    ...
    if (settings) {
      await db.execute(
        ...
        [data.defaultInterestRate, data.defaultProcessingFee, data.maxLoanAmount, data.minLoanAmount, data.defaultCurrency || "INR", data.financialYear || "April-March", data.enableSMS ? 1 : 0, data.enableEmail ? 1 : 0, data.enableWhatsApp ? 1 : 0],
      );
    } else {
      await db.execute(
