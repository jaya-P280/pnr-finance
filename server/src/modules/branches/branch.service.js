...
class BranchService {
    ...
    generateBranchCode(lastBranchCode) {
        ...
        const number = Number(
            lastBranchCode.replace(BRANCH.PREFIX, "")
        );
    ...
    async createBranch(data, currentUser) {
        ...
        try {
            ...
            if (email) {
                ...
                throw new ApiError(
                    409,
                    BRANCH_MESSAGES.EMAIL_EXISTS
                );

            }

            const phone =
                await branchRepository.existsByPhone(
                    connection,
                    data.phone
                );
            ...
            if (phone) {
                ...
                throw new ApiError(
                    409,
                    BRANCH_MESSAGES.PHONE_EXISTS
                );

            }

            const lastBranch =
                await branchRepository.getLastBranchCode();
            ...
            await branchRepository.rollback(
                connection
            );
    ...
    async getBranches(query) {
        ...
        const branches =
            await branchRepository.getBranches(filters);

        const totalRecords =
            await branchRepository.countBranches(filters);
    ...
    async getBranchById(branchId) {
        ...
        const branch =
            await branchRepository.getBranchById(branchId);

        if (!branch) {

            throw new ApiError(
                404,
                BRANCH_MESSAGES.NOT_FOUND
            );
    ...
    async updateBranch(
        ...
        try {
            ...
            const email =
                await branchRepository.findByEmail(data.email);

            if (
                email &&
                email.branch_id !== branchId
            ) {
            ...
            const phone =
                await branchRepository.findByPhone(data.phone);
