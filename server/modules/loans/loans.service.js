  ...
  async createLoan(
    ...
    try {
      ...
      if (currentUser?.role_name === "CUSTOMER") {
        throw new ApiError(403, "Customers are not authorized to disburse loans.");
      }

      if (currentUser?.role_name === "BRANCH_MANAGER" || currentUser?.role_name === "FIELD_OFFICER") {
        if (currentUser.branch_id && application.branch_id && application.branch_id !== currentUser.branch_id) {
          throw new ApiError(403, "You are only authorized to disburse loans for your own branch.");
        }
      }
