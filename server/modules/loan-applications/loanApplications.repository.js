...
class LoanApplicationRepository {
  ...
  async getGroupMembers(groupId) {
    const [rows] = await db.execute(
      `SELECT gm.*, CONCAT(c.first_name, ' ', COALESCE(c.last_name, '')) AS customer_name, c.customer_code, c.status AS customer_status
       FROM group_members gm
       INNER JOIN customers c ON c.customer_id = gm.customer_id
       WHERE gm.group_id = ? AND gm.deleted_at IS NULL AND c.deleted_at IS NULL`,
      [groupId],
    );
    return rows;
  }

  async isCustomerInGroup(groupId, customerId) {
    const [rows] = await db.execute(
