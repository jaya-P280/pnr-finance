  ...
  async createLoanApplication(data, currentUser) {
    ...
    try {
      ...
      if (data.groupId) {
        ...
        // Validate group ownership on backend (never trust group_id from frontend)
        if (currentUser?.role_name === "FIELD_OFFICER") {
          if (group.field_officer_id && Number(group.field_officer_id) !== Number(currentUser.user_id)) {
            throw new ApiError(403, "Access denied: Selected group is not assigned to you.");
          }
          if (currentUser.branch_id && Number(group.branch_id) !== Number(currentUser.branch_id)) {
            throw new ApiError(403, "Access denied: Selected group belongs to another branch.");
          }
        } else if (currentUser?.role_name === "BRANCH_MANAGER") {
          if (currentUser.branch_id && Number(group.branch_id) !== Number(currentUser.branch_id)) {
            throw new ApiError(403, "Access denied: Selected group belongs to another branch.");
          }
        }

        const members = await loanApplicationRepository.getGroupMembers(data.groupId);
        if (!members || members.length === 0) {
          throw new ApiError(400, "Cannot create loan application: Selected group has no active members.");
        }

        if (data.customerId) {
          const isMember = await loanApplicationRepository.isCustomerInGroup(data.groupId, data.customerId);
          if (!isMember) {
            throw new ApiError(400, "Selected customer is not a member of this group.");
          }
        }
      }
