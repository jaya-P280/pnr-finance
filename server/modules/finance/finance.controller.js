...
class FinanceController {
  ...
  getExpenses = async (req, res, next) => {
    try {
      ...
      if (!query.branchId && req.user && !["SUPER_ADMIN", "ADMIN"].includes(req.user.role_name)) {
        if (req.user.branch_id) query.branchId = req.user.branch_id;
      }
  ...
  getIncome = async (req, res, next) => {
    try {
      ...
      const result = await financeService.getIncome(query);
  ...
  getCashBook = async (req, res, next) => {
    try {
      ...
      const result = await financeService.getCashBook(query);
