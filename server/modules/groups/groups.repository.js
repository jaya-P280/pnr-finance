...
class GroupRepository {
  ...
  async create(connection, data) {
    const [result] = await connection.execute(
      `INSERT INTO customer_groups (group_code, group_name, branch_id, field_officer_id, description, meeting_day, status, created_by)
       VALUES (?,?,?,?,?,?,?,?)`,
      [data.groupCode, data.groupName, data.branchId, data.fieldOfficerId || null, data.description || null, data.meetingDay || null, data.status || "ACTIVE", data.createdBy],
    );
  ...
  async findAll(filters) {
    ...
    let sql = `
      SELECT cg.*, b.branch_name,
        CONCAT(fo.first_name, ' ', COALESCE(fo.last_name, '')) AS field_officer_name,
      ...
      LEFT JOIN users fo ON fo.user_id = cg.field_officer_id
    ...
    if (filters.fieldOfficerId) {
      sql += ` AND cg.field_officer_id = ?`;
      params.push(filters.fieldOfficerId);
    }
    if (filters.status) {
      sql += ` AND cg.status = ?`;
    ...
    params.push(limit, Math.max(0, (page - 1) * limit));
  ...
  async count(filters) {
    ...
    if (filters.fieldOfficerId) { sql += ` AND cg.field_officer_id = ?`; params.push(filters.fieldOfficerId); }
  ...
  async update(connection, id, data) {
    await connection.execute(
      `UPDATE customer_groups SET group_name=?, branch_id=?, field_officer_id=?, description=?, meeting_day=?, status=?, updated_by=? WHERE group_id=?`,
      [data.groupName, data.branchId, data.fieldOfficerId || null, data.description || null, data.meetingDay || null, data.status || "ACTIVE", data.updatedBy, id],
    );
  }
