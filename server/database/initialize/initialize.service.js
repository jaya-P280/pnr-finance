...
class InitializeService {
  async initialize() {
    ...
    try {
      ...
      // ===== 5. Create Default Branch (Only if missing) =====
      logger.info("Checking Head Office branch...");
      ...
      logger.info("Checking users table for existing staff/admins...");
      const sampleUsers = [
        {
      ...
      // ===== 13. Seed Employee Salaries, Payroll & Attendance (ONLY IF TABLE IS EMPTY) =====
      const [salCount] = await connection.execute(`SELECT COUNT(*) AS total FROM employee_salaries`);
      if (salCount[0]?.total === 0) {
        ...
        logger.info("Employee Salaries, Payroll Logs, and Attendance seeded because table was empty");
      } else {
        logger.info(`Employee salaries table has ${salCount[0]?.total} records; sample seeding skipped.`);
      }

      // ===== 14. Seed HR Letters (ONLY IF TABLE IS EMPTY) =====
      ...
      if (letterCount[0]?.total === 0) {
        ...
        logger.info("Sample HR Letters seeded because table was empty");
      } else {
        logger.info(`Letters table has ${letterCount[0]?.total} records; sample seeding skipped.`);
      }

      await repository.commit(connection);
      logger.info("Database initialization check completed cleanly.");
    } catch (error) {
